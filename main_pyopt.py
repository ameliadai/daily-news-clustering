...
def plot_cluster(valid_cluster_inds, X, data, topk):
    ...
    """
    # reduce to 2D (UMAP generally faster for large datasets)
    ...
    top_valid = sorted(valid_cluster_inds, 
                        ...
                        reverse=True)[:topk]
    wc_title_fig, axs_title = plt.subplots(topk, 1,
                                           figsize=(8, 3*topk))
    wc_text_fig, axs_text = plt.subplots(topk, 1,
                                         figsize=(8, 3*topk))
    for i in range(topk):
...
@time_func
def process_date(date, data, output_path, topk):
    """
...
def main(args):
    ...
    for current_date in tqdm(date_range, desc="Processing Dates"):
        ...
        process_time, times = process_date(date_str, data, args.output_path, args.topk) 
