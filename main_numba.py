...
def cosine_distance_matrix(X):
    n_samples = X.shape[0]
    result = np.empty((n_samples, n_samples), dtype=np.float32)
    ...
    for i in prange(n_samples):
        for j in prange(n_samples):
            ...
            denom = norm_i * norm_j if norm_i * norm_j != 0 else 1e-8
            result[i, j] = 1.0 - dot / denom
    return result

@njit(cache=True)
def kth_neighbor_distances(X, k):
    n = X.shape[0]
...
def select_top_articles(data, labels, X, avg_distance_threshold=0.6):
    ...
    for cluster_id, indices in clusters.items():
        ...
        distances = cosine_distance_matrix(cluster_points) # NUMBA OPTIMIZATION
        avg_distance = np.sum(distances) / (distances.shape[0] * distances.shape[1])
